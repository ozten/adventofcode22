...
pub fn parse(filename: &str) -> Vec<Monkey> {
    ...
    for line in read_to_string(filename).unwrap().lines() {
        ...
        if !line.is_empty() {
            match line {
                line if line.starts_with("Monkey ") => monkeys.push(Monkey::new()),
                ...
                line if line.starts_with("  Starting items: ") => {
                    ...
                    monkeys.get_mut(last_idx).unwrap().set_items(
                        numbers
                            .split(", ")
                            .map(|part| -> isize { part.trim().parse().unwrap() })
                            .collect(),
                    );
                }
